...
func (i Series) Read(ctx context.Context, params series.Params) (series.Set, error) {
	...
	secure := i.conf.HTTPConfig.TLSConfig.CertFile != "" ||
		i.conf.HTTPConfig.TLSConfig.KeyFile != "" ||
		i.conf.HTTPConfig.TLSConfig.CAFile != ""
	dialOpts, err := extgrpc.StoreClientGRPCOpts(i.logger, nil, tracing.NoopTracer(),
		secure,
		i.conf.HTTPConfig.TLSConfig.InsecureSkipVerify,
		i.conf.HTTPConfig.TLSConfig.CertFile,
		i.conf.HTTPConfig.TLSConfig.KeyFile,
		i.conf.HTTPConfig.TLSConfig.CAFile,
		i.conf.Endpoint,
	)
